...
state_diffs = True
grid_beta = 0.1
batch_generator_hipmdp = SiPMDP(domain,preset_hidden_params,
...
with open('results/simple_{}_exp_buffer'.format(domain),'wb') as f:
        pickle.dump(exp_buffer,f)

# with open('results/simple_{}_exp_buffer'.format(domain),'rb') as f:
...
# Initialize BNN
network = SimpleNetwork(param_set, nonlinearity=relu)
...
with open('results/simple_{}_network_weights'.format(domain), 'wb') as f:
...
for run in range(5):
    for test_inst in [0,1]:
        test_hipmdp = SiPMDP(domain,preset_hidden_params, 
                         ddqn_learning_rate=ddqn_learning_rate, 
                         episode_count=episode_count,
                         run_type=run_type,
        ...
        with open('results/simple_{}_results'.format(domain),'wb') as f:
            pickle.dump(results,f)

# with open('results/simple_{}_results'.format(domain),'rb') as f:
...
def plot_results(clean_results, test_inst):
    ...
    plt.savefig('results/simple_{}_results_{}.png'.format(domain, test_inst))
