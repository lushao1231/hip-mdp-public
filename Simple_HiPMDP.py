...
class SiPMDP(object):
	...
	def __init__(self, domain, preset_hidden_params, run_type='full', ddqn_learning_rate=0.0001, 
		...
		bnn_network_weights -- 1-D numpy array of pretrained BNN network weights (default=None)
	...
	def __initialize_BNN(self):
		...
		if self.run_type != 'full_linear':
			self.network = SimpleNetwork(param_set, nonlinearity=relu)
		else:
			self.network = SimpleNetwork(param_set, nonlinearity=relu, linear_latent_weights=True)
