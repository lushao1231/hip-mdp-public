...
class Qnetwork():
	...
	def __init__(self, state_dims, num_actions, clip = 2.5, activation_fn=tf.nn.relu, hidden_layer_sizes=(256,512)):
		...
		self.s = tf.compat.v1.placeholder(tf.float32,[None,state_dims])
		self.next_Q = tf.compat.v1.placeholder(shape=[None],dtype=tf.float32)
		self.action_array = tf.compat.v1.placeholder(shape=[None,num_actions],dtype=tf.float32)
		self.importance_weights = tf.compat.v1.placeholder(shape=[None],dtype=tf.float32)
		...
		# Hidden layer 1
		self.h1_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([state_dims,self.hidden1_units], stddev=1.0/float(state_dims*self.hidden1_units)), name='weights')
		self.h1_biases = tf.compat.v1.Variable(tf.zeros([self.hidden1_units]))
		self.hidden1 = self.activation_fn(tf.matmul(self.s,self.h1_weights) + self.h1_biases)
		# Hidden layer 2
		self.h2_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([self.hidden1_units, self.hidden2_units], stddev=1.0/float(self.hidden1_units*self.hidden2_units)),name='weights')
		self.h2_biases = tf.compat.v1.Variable(tf.zeros([self.hidden2_units]))
		self.hidden2 = self.activation_fn(tf.matmul(self.hidden1,self.h2_weights) + self.h2_biases)
		# Output layer
		self.out_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([self.hidden2_units, num_actions], stddev=1.0/float(self.hidden2_units*num_actions)),name='weights')
		self.out_biases = tf.compat.v1.Variable(tf.zeros([num_actions]))
		...
		if clip > 0: # Train and backpropagate clipped errors
			...
			self.objective = tf.compat.v1.train.AdamOptimizer(learning_rate = self.learning_rate,epsilon=0.01) 
			...
			self.updateQ = tf.compat.v1.train.AdamOptimizer(learning_rate = self.learning_rate).minimize(self.loss)
